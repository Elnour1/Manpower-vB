...
// 📲 HR WHATSAPP CONFIGURATION
const HR_WHATSAPP_NUMBER = "+966570544368";
...
// 📌 مسار خاص لضمان فتح صفحة التخطيط بدون أخطاء 404
...
// -------------------------------------------------------------
// 📊 DASHBOARD & WHATSAPP APIs
// -------------------------------------------------------------

// 📲 API إرسال التنبيهات وإشعار الواتساب لـ HR
app.post('/api/dispatch/whatsapp', async (req, res) => {
  const { shift, totalWorkers, actionText } = req.body;
  
  await logAudit('HR_DISPATCH', 'WHATSAPP_NOTIFY', `Sent order for ${shift} to ${HR_WHATSAPP_NUMBER}`);
  
  res.json({ 
    success: true, 
    targetPhone: HR_WHATSAPP_NUMBER,
    message: `HR WhatsApp dispatch notification queued for ${HR_WHATSAPP_NUMBER}` 
  });
});
